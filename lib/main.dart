      ...
      client.addListener((message) {
        setState(() {
          switch (message) {
            ...
            case PIMPTransactionOtherPropertyRemovedMessage(
              transactionID: int transactionID,
              property: int property,
            ):
              transactions[transactionID]!.otherProperties.remove(property);
            ...
            case PIMPGotCardMessage(card: int card, player: int player):
              ...
              cardReceived = card;
            case PIMPDeltaBankMessage(houses: int houses, hotels: int hotels):
              bankHouses = houses;
              bankHotels = hotels;
            default:
              print(message);
                                  ...
                                  child: ListView(
                                    children: [
                                      ...
                                      if (bankHouses != null)
                                        Center(
                                          child: Text(
                                            'The bank has $bankHouses houses and $bankHotels hotels.',
                                          ),
                                        ),
        ...
        child: Dialog(
          child: Padding(
            ...
            child: Column(
              children: [
                Text(
                  'Transaction ${widget.transaction.transactionID} with ${widget.players[widget.transaction.otherPlayer]?.name ?? (widget.transaction.otherPlayer == 0 ? 'Bank' : 'ERROR: Unknown player')}: ${widget.transaction.reason}',
                ),
                if (widget.transaction.cash != null)
                  Text('You are offering \$${widget.transaction.cash}.'),
                for (int property in widget.transaction.properties)
                  Text(
                                ...
                                child: SingleChildScrollView(
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Text('Offer property:'),
                                      for (Property property
                                          in widget.ownedProperties.values
                                              .where(
                                                (e) => e.owner == widget.player,
                                              )) ...[
                                        SizedBox(
                                          width: 150,
                                          child: PropertyWidget(
                            ...
                            child: Padding(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text('Offer card:'),
                                  for (Card card in widget.cards.values.where(
                                    (e) => e.owner == widget.player,
                                  )) ...[
                                    CardWidget(
                                      card: card,
                ...
                else if (!widget.transaction.agreed)
                  Padding(
                ...
                if (widget.transaction.cancellable)
                  Padding(
                    padding: const EdgeInsets.fromLTRB(0, 8, 0, 0),
                    child: OutlinedButton(
                      onPressed: () async {
                ...
                Padding(
                  ...
                  child: OutlinedButton(
                    onPressed: () async {
                      print(
                        await widget.client.sendMessage(
                          PIMPBankruptTransactionMessage(
                            widget.transaction.transactionID,
                          ),
                          [0xfe, 0xea, 0xfc],
                          true,
